# board-login-img-security-Oauth — 실제 만드는 순서
...
1. Spring Boot **4.0.x** 부모 POM, `artifactId`를 `board-login-img-security-Oauth` 등으로 구분
2. 의존성 추가·유지  
...
1. **`src/main/resources/application.properties`**  
   ...
   - MySQL `datasource` (DB 이름은 원본과 겹치지 않게, 예: `board_login_img_security_oauth`)  
...
- **`BoardLoginImgSecurityOauthApplication.java`** — `@SpringBootApplication`, `main`
...
- **`docs/board-login-img-vs-board-login-img-security-Oauth.md`** — 원본과 차이 정리
