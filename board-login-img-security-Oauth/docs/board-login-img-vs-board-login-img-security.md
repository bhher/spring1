# board-login-img vs board-login-img-security-Oauth — 추가·변경 사항

`board-login-img-security-Oauth`는 `board-login-img`를 복사한 뒤 **Spring Security**를 붙인 프로젝트입니다. 게시판·이미지 업로드·JPA 구조는 같고, **인증 방식과 보안 관련 설정·코드**가 바뀝니다.
...
`artifactId` / 앱 이름은 `board-login-img-security-Oauth`로 구분됩니다.
...
| `BoardLoginImgSecurityOauthApplication.java` | 진입점 (패키지 `com.example.boardloginimgsecurityoauth`) |

---

## 3. 제거·대체된 부분

| board-login-img | board-login-img-security-Oauth |
|-----------------|---------------------------|
...
| 항목 | board-login-img | board-login-img-security-Oauth |
|------|-----------------|---------------------------|
| 앱 이름 | `board-login-img` | `board-login-img-security-Oauth` |
| 포트 | `8080` | `8082` (원본과 동시 실행 시 충돌 완화) |
| MySQL DB 이름 | `board_login_img` | `board_login_img_security_oauth` |
| 업로드 폴더 | `~/board-login-img-uploads/` | `~/board-login-img-security-Oauth-oauth-uploads/` |
...
**board-login-img-security-Oauth = board-login-img + Spring Security 7(폼 로그인·BCrypt·URL 권한·CSRF) − 세션 수동 로그인/인터셉터**, 패키지·DB·포트·업로드 경로는 별도 프로젝트로 겹치지 않게 조정된 버전입니다. (Spring Boot 4.0.x 부모 POM 기준.)
