...
public class CustomOAuth2UserService extends DefaultOAuth2UserService {
	...
	private OAuthUserProfile extractProfile(String registrationId, Map<String, Object> attrs) {
		return switch (registrationId) {
			case "google" -> new OAuthUserProfile(
					...
					firstNonBlank((String) attrs.get("name"), "구글사용자"));
			case "naver" -> {
				...
				String name = firstNonBlank((String) response.get("name"), (String) response.get("email"));
				yield new OAuthUserProfile(
						String.valueOf(response.get("id")),
						(String) response.get("email"),
						name,
						firstNonBlank((String) response.get("nickname"), name));
			}
			case "kakao" -> {
				...
				yield new OAuthUserProfile(id, email, nickname, nickname);
			}
	...
	private static String firstNonBlank(String a, String b) {
		...
		return b != null && !b.isBlank() ? b : "user";
	}
}
