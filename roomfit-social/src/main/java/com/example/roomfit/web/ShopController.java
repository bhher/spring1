...
public class ShopController {
	...
	@GetMapping("/{id}")
	public String detail(@PathVariable("id") Long id, @AuthenticationPrincipal(expression = "member") Member member, Model model) {
	...
	@PostMapping("/{id}/cart")
	public String addCart(@PathVariable("id") Long id, @AuthenticationPrincipal(expression = "member") Member member) {
		shopService.addToCart(member.getId(), id, 1);
		return "redirect:/shop/cart";
	}

	@GetMapping("/cart")
	public String cart(@AuthenticationPrincipal(expression = "member") Member member, Model model) {
		model.addAttribute("cart", shopService.getCartWithItems(member.getId()));
		return "shop/cart";
	}

	@PostMapping("/{id}/wish")
	public String wish(@PathVariable("id") Long id, @AuthenticationPrincipal(expression = "member") Member member) {
